...
constexpr static const int chamber_10deg_bw = 52;        // 13.8667 deg
constexpr static const int chamber_20deg_bw = 90;        // 24 deg
constexpr static const int chamber_70deg_bw = 315 - 38;  // 73.8667 deg
constexpr static const int chamber_80deg_bw = 315;       // 84 deg
...
constexpr static const int chambers_zone_1_row_5_0[4] = { 27,  28,  29,  50};                 // ME3/1
constexpr static const int chambers_zone_1_row_5_1[7] = { 30,  31,  32,  33,  34,  35,  51};  // ME3/2
constexpr static const int chambers_zone_1_row_6_0[4] = { 81,  82,  83, 104};                 // RE3/1
constexpr static const int chambers_zone_1_row_6_1[7] = { 84,  85,  86,  87,  88,  89, 105};  // RE3/2
constexpr static const int chambers_zone_1_row_7_0[4] = { 36,  37,  38,  52};                 // ME4/1
constexpr static const int chambers_zone_1_row_7_1[7] = { 39,  40,  41,  42,  43,  44,  53};  // ME4/2
constexpr static const int chambers_zone_1_row_7_2[4] = { 90,  91,  92, 106};                 // RE4/1
constexpr static const int chambers_zone_1_row_7_3[7] = { 93,  94,  95,  96,  97,  98, 107};  // RE4/2
...
constexpr static const int chamber_70deg_ph_init = chambers_10deg_ph_init[7 - 1];
constexpr static const int chamber_80deg_ph_init = chambers_20deg_ph_init[4 - 1];
...
struct zoning_10deg_chamber_traits {
  static const int chtype = detail::chamber_10deg_chtype;
  static const int nch = detail::chamber_10deg_nch;
  static const int chamber_img_bw = detail::chamber_10deg_bw;
  static const int chamber_img_joined_bw = detail::chamber_70deg_bw;
  static const int chamber_img_joined_ph_init = detail::chamber_70deg_ph_init;
  static const int chamber_img_joined_col_start = AP_MAX(0, detail::chamber_288unit_ph_init - chamber_img_joined_ph_init);
  static const int chamber_img_joined_col_stop = AP_MIN(detail::chamber_288unit_ph_cover, chamber_img_joined_bw - 1);
  static const int chamber_img_resized_col_start = AP_MAX(0, chamber_img_joined_ph_init - detail::chamber_288unit_ph_init);
  static const int chamber_img_resized_col_stop = AP_MIN(num_img_cols, chamber_img_resized_col_start + chamber_img_joined_col_stop - chamber_img_joined_col_start);
  typedef ap_uint<chamber_img_bw> chamber_img_t;
  typedef ap_uint<chamber_img_joined_bw> chamber_img_joined_t;
};

struct zoning_20deg_chamber_traits {
  static const int chtype = detail::chamber_20deg_chtype;
  static const int nch = detail::chamber_20deg_nch;
  static const int chamber_img_bw = detail::chamber_20deg_bw;
  static const int chamber_img_joined_bw = detail::chamber_80deg_bw;
  static const int chamber_img_joined_ph_init = detail::chamber_80deg_ph_init;
  static const int chamber_img_joined_col_start = AP_MAX(0, detail::chamber_288unit_ph_init - chamber_img_joined_ph_init);
  static const int chamber_img_joined_col_stop = AP_MIN(detail::chamber_288unit_ph_cover, chamber_img_joined_bw - 1);
  static const int chamber_img_resized_col_start = AP_MAX(0, chamber_img_joined_ph_init - detail::chamber_288unit_ph_init);
  static const int chamber_img_resized_col_stop = AP_MIN(num_img_cols, chamber_img_resized_col_start + chamber_img_joined_col_stop - chamber_img_joined_col_start);
...
template <> struct zoning_num_rows_traits<1> { static const int value = 1 + 1 + 2 + 1 + 1 + 2 + 2 + 4; };
...
template <> struct zoning_row_traits<1, 6>  : zoning_20deg_chamber_traits {};
template <> struct zoning_row_traits<1, 7>  : zoning_10deg_chamber_traits {};
template <> struct zoning_row_traits<1, 8>  : zoning_20deg_chamber_traits {};
template <> struct zoning_row_traits<1, 9>  : zoning_10deg_chamber_traits {};
template <> struct zoning_row_traits<1, 10> : zoning_20deg_chamber_traits {};
template <> struct zoning_row_traits<1, 11> : zoning_10deg_chamber_traits {};
template <> struct zoning_row_traits<1, 12> : zoning_20deg_chamber_traits {};
template <> struct zoning_row_traits<1, 13> : zoning_10deg_chamber_traits {};
...
template <> struct zoning_get_chamber_op<1, 6>  { inline int operator ()(int i) const { return detail::chambers_zone_1_row_5_0[i]; } };
template <> struct zoning_get_chamber_op<1, 7>  { inline int operator ()(int i) const { return detail::chambers_zone_1_row_5_1[i]; } };
template <> struct zoning_get_chamber_op<1, 8>  { inline int operator ()(int i) const { return detail::chambers_zone_1_row_6_0[i]; } };
template <> struct zoning_get_chamber_op<1, 9>  { inline int operator ()(int i) const { return detail::chambers_zone_1_row_6_1[i]; } };
template <> struct zoning_get_chamber_op<1, 10> { inline int operator ()(int i) const { return detail::chambers_zone_1_row_7_0[i]; } };
template <> struct zoning_get_chamber_op<1, 11> { inline int operator ()(int i) const { return detail::chambers_zone_1_row_7_1[i]; } };
template <> struct zoning_get_chamber_op<1, 12> { inline int operator ()(int i) const { return detail::chambers_zone_1_row_7_2[i]; } };
template <> struct zoning_get_chamber_op<1, 13> { inline int operator ()(int i) const { return detail::chambers_zone_1_row_7_3[i]; } };
...
#pragma HLS UNROLL
      ...
      if (
        ...
        chamber_img.set(col);  // set bit to 1
      }
    }  // end loop over segments

    // Join chamber images
    assert(chambers_ph_init[i] >= (zoning_row_traits<ZONE, ROW>::chamber_img_joined_ph_init));
    const zoning_col_t col_start_rhs = 0;
    const zoning_col_t col_stop_rhs = chamber_img_t::width - 1;
    const zoning_col_t col_start_lhs = chambers_ph_init[i] - (zoning_row_traits<ZONE, ROW>::chamber_img_joined_ph_init);
    ...
    auto chamber_img_tmp = chamber_img_joined.range(col_stop_lhs, col_start_lhs);
    chamber_img_joined.range(col_stop_lhs, col_start_lhs) = (chamber_img.range(col_stop_rhs, col_start_rhs) | chamber_img_tmp);
  ...
  const zoning_col_t col_start_lhs = zoning_row_traits<ZONE, ROW>::chamber_img_resized_col_start;
  const zoning_col_t col_stop_lhs = zoning_row_traits<ZONE, ROW>::chamber_img_resized_col_stop;
...
#pragma HLS INLINE region
  ...
  zoning_out_t zoning_out_tmp[zoning_num_rows_traits<ZONE>::value];

#pragma HLS ARRAY_PARTITION variable=zoning_out_tmp complete dim=0

  // Loop over chamber_rows manually
  zoning_row_op<ZONE, 0>(emtf_phi, flags_zone, flags_tzone, valid, zoning_out_tmp[0]);
  zoning_row_op<ZONE, 1>(emtf_phi, flags_zone, flags_tzone, valid, zoning_out_tmp[1]);
  zoning_row_op<ZONE, 2>(emtf_phi, flags_zone, flags_tzone, valid, zoning_out_tmp[2]);
  zoning_row_op<ZONE, 3>(emtf_phi, flags_zone, flags_tzone, valid, zoning_out_tmp[3]);
  zoning_row_op<ZONE, 4>(emtf_phi, flags_zone, flags_tzone, valid, zoning_out_tmp[4]);
  zoning_row_op<ZONE, 5>(emtf_phi, flags_zone, flags_tzone, valid, zoning_out_tmp[5]);
  zoning_row_op<ZONE, 6>(emtf_phi, flags_zone, flags_tzone, valid, zoning_out_tmp[6]);
  zoning_row_op<ZONE, 7>(emtf_phi, flags_zone, flags_tzone, valid, zoning_out_tmp[7]);
  zoning_row_op<ZONE, 8>(emtf_phi, flags_zone, flags_tzone, valid, zoning_out_tmp[8]);

  // Loop over img_rows manually
...
#pragma HLS ARRAY_PARTITION variable=zoning_out_tmp complete dim=0
  ...
  zoning_row_op<ZONE, 7>(emtf_phi, flags_zone, flags_tzone, valid, zoning_out_tmp[7]);
  zoning_row_op<ZONE, 8>(emtf_phi, flags_zone, flags_tzone, valid, zoning_out_tmp[8]);
  zoning_row_op<ZONE, 9>(emtf_phi, flags_zone, flags_tzone, valid, zoning_out_tmp[9]);
  zoning_row_op<ZONE, 10>(emtf_phi, flags_zone, flags_tzone, valid, zoning_out_tmp[10]);
  zoning_row_op<ZONE, 11>(emtf_phi, flags_zone, flags_tzone, valid, zoning_out_tmp[11]);
  zoning_row_op<ZONE, 12>(emtf_phi, flags_zone, flags_tzone, valid, zoning_out_tmp[12]);
  zoning_row_op<ZONE, 13>(emtf_phi, flags_zone, flags_tzone, valid, zoning_out_tmp[13]);

  // Loop over img_rows manually
  zoning_out[0] = zoning_out_tmp[0];
  zoning_out[1] = zoning_out_tmp[1];
  zoning_out[2] = (zoning_out_tmp[2] | zoning_out_tmp[3]);
  zoning_out[3] = zoning_out_tmp[4];
  zoning_out[4] = zoning_out_tmp[5];
  zoning_out[5] = (zoning_out_tmp[6] | zoning_out_tmp[7]);
  zoning_out[6] = (zoning_out_tmp[8] | zoning_out_tmp[9]);
  zoning_out[7] = (zoning_out_tmp[10] | zoning_out_tmp[11] | zoning_out_tmp[12] | zoning_out_tmp[13]);
}
...
#pragma HLS INLINE region
  ...
  static_assert(zoning_num_rows_traits<1>::value == 14, "num_rows (zone 1) must be 14");
